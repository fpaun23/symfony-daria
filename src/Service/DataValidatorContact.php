...
{    
    ...
    {
        if ((strlen($val[0]) < ConstantContact::NAME[0] || 
            strlen($val[0]) > ConstantContact::NAME[1]) || 
            !preg_match("/^[a-zA-Z]+$/", $val[0])) 
        {
        ...
        if (strlen($val[1]) < ConstantContact::DESC[0] || 
            strlen($val[1]) > ConstantContact::DESC[1]) 
        {
